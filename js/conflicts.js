/**
 * conflicts.js
 * Step 2: Conflict Detection Module
 * Detects conflicts in extracted facts based on business rules
 */

const Conflicts = {
    /**
     * Generate conflicts from extracted facts
     * Detects contradictions in budgets, timelines, priorities, and requirements
     * @param {array} facts - Array of extracted facts
     * @returns {array} Array of conflict objects
     */
    detectConflicts(facts) {
        const conflicts = [];
        const processedPairs = new Set(); // Track processed fact pairs
        let conflictId = 1;
        ...
        const budgetFacts = facts.filter(f => 
        ...
        for (let i = 0; i < budgetFacts.length; i++) {
            for (let j = i + 1; j < budgetFacts.length; j++) {
                ...
                // Extract amounts
                const amounts1 = this.extractAmounts(budgetFacts[i].content);
                const amounts2 = this.extractAmounts(budgetFacts[j].content);
                ...
                if (amounts1.length > 0 && amounts2.length > 0) {
                    ...
                    if (Math.abs(amounts1[0] - amounts2[0]) > 5000) {
                        conflicts.push({
                            ...
                            selectedFactId: null,
                            comment: '',
                            description: `Budget discrepancy: ${this.formatCurrency(amounts1[0])} vs ${this.formatCurrency(amounts2[0])}`
                        });
                        processedPairs.add(pairKey);
                        break; // Only one budget conflict
                    }
                }
            }
        }

        // Audit/Security conflict detection
        const auditFacts = facts.filter(f => 
        ...
        // Timeline/Launch conflict detection
        const timelineFacts = facts.filter(f => 
        ...
        for (let i = 0; i < timelineFacts.length && !timelineConflictFound; i++) {
            for (let j = i + 1; j < timelineFacts.length && !timelineConflictFound; j++) {
                ...
                const hasUrgent1 = timelineFacts[i].content.toLowerCase().includes('non-negotiable') || 
                                  timelineFacts[i].content.toLowerCase().includes('locked') ||
                                  timelineFacts[i].content.toLowerCase().includes('friday');
                const hasDelay2 = (timelineFacts[j].content.toLowerCase().includes('need') && timelineFacts[j].content.toLowerCase().includes('days')) ||
        ...
        // Feature priority conflict detection
        const featureFacts = facts.filter(f => 
        ...
        for (let i = 0; i < featureFacts.length && !featureConflictFound; i++) {
            for (let j = i + 1; j < featureFacts.length && !featureConflictFound; j++) {
                ...
                const hasPriority1 = featureFacts[i].content.toLowerCase().includes('priority') || 
                                    featureFacts[i].content.toLowerCase().includes('dealbreaker') ||
                                    featureFacts[i].content.toLowerCase().includes('top');
                const hasSecondary2 = featureFacts[j].content.toLowerCase().includes('secondary') || 
        ...
        const techFacts = facts.filter(f => 
        ...
        for (let i = 0; i < techFacts.length && !techConflictFound; i++) {
            for (let j = i + 1; j < techFacts.length && !techConflictFound; j++) {
                ...
                const hasPerformance1 = techFacts[i].content.toLowerCase().includes('faster') || 
                                       techFacts[i].content.toLowerCase().includes('10x') ||
                                       techFacts[i].content.toLowerCase().includes('free');
                const hasSecurity2 = techFacts[j].content.toLowerCase().includes('cve') || 
    ...
    extractAmounts(text) {
        const amounts = [];
        // Match patterns like $45k, $55,000, $80k
        const patterns = [
            /\$(\d+)k/gi,
            /\$(\d+,\d+)/g,
            /\$(\d+)/g
     ...
     * @param {string} selectedFactId - ID of selected fact
     * @param {string} comment - Optional comment
     * @returns {object} Updated conflict
     */
    resolveConflict(conflict, selectedFactId, comment = '') {
        return {
            ...conflict,
            resolved: true,
     ...
     * @returns {boolean} True if all resolved
     */
    allResolved(conflicts) {
...
// Make Conflicts available globally
